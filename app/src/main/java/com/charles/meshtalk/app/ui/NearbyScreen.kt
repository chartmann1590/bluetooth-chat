...
@Composable
fun NearbyScreen(repository: MeshRepository, onMessagePeer: (String) -> Unit, onFindPeer: (String) -> Unit) {
            ...
            LazyColumn {
                items(contacts, key = { it.signingPubKeyHex }) { contact ->
                    Row(
                        ...
                        IconButton(onClick = { onFindPeer(contact.signingPubKeyHex) }) {
                            Icon(Icons.Filled.GpsFixed, contentDescription = "Find ${contact.nickname}")
                        }
                    }
                    HorizontalDivider()
                }
